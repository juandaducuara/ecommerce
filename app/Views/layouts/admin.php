...
<head>
    ...
    <title><?= esc($title ?? 'Admin') ?> — <?= esc($_storeName) ?></title>
    ...
    <aside id="sidebar"
        ...
        <div class="px-6 py-5 border-b border-gray-700 flex items-center justify-between">
            <a href="/admin" class="flex items-center gap-2 min-w-0">
                <?php if ($_storeLogo): ?>
                    <img src="/<?= esc($_storeLogo) ?>" alt="<?= esc($_storeName) ?>" class="h-8 w-auto object-contain flex-shrink-0">
                <?php endif; ?>
                <span class="text-sm font-bold text-white tracking-tight truncate"><?= esc($_storeName) ?></span>
            </a>
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            <?php
                ...
                $isReports = str_starts_with($currentUri, '/admin/reports');
            ?>
            ...
            ?>
                <a href="<?= $item['href'] ?>"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition
                          <?= $isActive ? 'brand-bg text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' ?>">
            ...
            <!-- Reportes (con sub-menú) -->
            <div>
                <a href="/admin/reports/sales"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition
                          <?= $isReports ? 'brand-bg text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' ?>">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="ml-8 mt-1 space-y-1">
                    ...
                    $reportItems = [
                        ...
                        '/admin/reports/products'  => 'Productos',
                        '/admin/reports/customers' => 'Clientes',
                        '/admin/reports/inventory' => 'Inventario',
                    ];
                    foreach ($reportItems as $href => $label):
                        $isSubActive = $currentUri === $href;
                    ?>
                        <a href="<?= $href ?>"
                           class="block px-3 py-1.5 rounded-lg text-xs font-medium transition
                                  <?= $isSubActive ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white' ?>">
                            <?= $label ?>
                        </a>
                    <?php endforeach; ?>
                </div>
