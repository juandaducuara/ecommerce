    ...
    <div class="bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition">
        ...
        <div class="p-4">
            <?php if (isset($product->category_name)): ?>
                <p class="text-xs text-indigo-600 font-medium mb-1"><?= esc($product->category_name) ?></p>
            <?php endif; ?>

            <h3 class="font-medium text-gray-800 group-hover:text-indigo-600 transition line-clamp-2">
                <?= esc($product->name) ?>
            </h3>
            ...
            <div class="mt-2 flex items-baseline gap-2">
                ...
                <?php if (isset($product->compare_price) && $product->compare_price > $product->price): ?>
                    <span class="text-sm text-gray-400 line-through">$<?= number_format($product->compare_price, 0, ',', '.') ?></span>
                <?php endif; ?>
            </div>
        </div>
    </div>
